...
if(isset($_POST['add_to_wishlist'])){
    ...
    if(mysqli_num_rows($check_wishlist_numbers) > 0){
        ...
        mysqli_query($conn, "INSERT INTO `wishlist`(user_id, pid, name, price, image) VALUES('$user_id', '$instruments_id', '$instruments_name', '$instruments_price', '$instruments_image')") or die('query failed');
   ...
   <div class="box-container">
      ...
      <?php
        if(isset($_POST['search_btn'])){
         ...
         $select_products = mysqli_query($conn, "SELECT * FROM `instruments` WHERE name LIKE '%{$search_box}%'") or die('query failed');
         if(mysqli_num_rows($select_instruments) > 0){
            while($fetch_instruments = mysqli_fetch_assoc($select_instruments)){
      ?>
      <form action="" method="POST" class="box">
         <a href="view_page.php?pid=<?php echo $fetch_instruments['id']; ?>" class="fas fa-eye"></a>
